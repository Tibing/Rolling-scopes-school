...
function logger(func, logFunc) {
    return function() {
        let answer,
            ...
            log = func.name + '(' + str.slice(1, str.length - 1) + ')';
...
function partialUsingArguments(fn) {
    let result = '',
        argsPart1 = Array.from(arguments);
    argsPart1.shift();

    while (argsPart1.length) {
        result += argsPart1.shift();
    }
    return function() {
        let argsPart2 = Array.from(arguments);
        while (argsPart2.length) {
            result += argsPart2.shift();
        }
        return result;
    }
}
